...
struct Version {
  ...
  public:
    ...
    Version() : _major(0), _minor(0), _revision(0), _build(0) {
        // do nothing
    }
    ...
    bool operator==(const Version &other) {
        return _major == other._major && _minor == other._minor &&
               _revision == other._revision && _build == other._build;
    }
    ...
    {
        out << other._major << "." << other._minor << "." << other._revision
            << "." << other._build;
        return out;
    }
  ...
  private:
    void assgin_from_string(string str) {
        try {
            std::sscanf(str.c_str(), "%d.%d.%d.%d", &_major, &_minor,
                        &_revision, &_build);
...
int main() {
   ...
   while(true) {
      ...
      if(v1 == v2) {
         cout << "Both the versions, " << str_v1 << " and " << str_v2 << " are
equal" << endl; } else if(v1 > v2) { cout << "Version " << str_v1 << " is
greater than version " << str_v2 << endl; } else { cout << "Version " << str_v1
<< " is less than version " << str_v2 << endl;
      }
   }
