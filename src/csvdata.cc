...
void CSVData::add_row(vector<string> row_data) {
    ...
    if ((int)new_row_size != m_cols && m_is_unified)
        m_is_unified = false;
    m_data.push_back(row_data);
    m_rows++;
    if (m_cols < (int)new_row_size)
        m_cols = new_row_size;
}
...
void CSVData::delete_row_if(
    function<bool(int, const vector<string> &, void *cbData)> cbFun,
    void *cbData) {
    ...
    if (rows_to_delete.size() > 0) {
        ...
        for (int row = 0; row < (int)rows_to_delete.size(); ++row)
...
void CSVData::delete_row_if(
    function<bool(int, int, const string &, void *cbData)> cbFun,
    void *cbData) {
    vector<int> rows_to_delete;

    for (int row = 0; row < (int)m_data.size(); ++row) {
        for (int col = 0; col < (int)m_data.at(row).size(); ++col) {
...
void CSVData::delete_row_if(function<bool(int, int, const string &)> cbFun) {
    function<bool(int, int, const string &, void *cbData)>
        simple_delete_row_if =
            [=](int row, int col, const string &val, void *cbData) {
...
void CSVData::delete_item(int row, int col) {
    if (row >= m_rows || row < 0)
        return;
    if (col >= (int)m_data.at(row).size() || col < 0)
        return;
...
void CSVData::_append_data(vector<vector<string>> &data) {
    if (data.empty())
        return;
...
void CSVData::_read_file(const string &filename, vector<vector<string>> &target,
                         int &cols) {
    ifstream input_file(filename.c_str());

    if (!input_file.is_open()) {
        cerr << "Unable to open file '" << filename
             << "'. Check if file exists and if you have the right permissions."
             << endl;
        return;
    }
    ...
    while (os_safe_getline(input_file, line)) {
        ...
        current_line++;
        if ((contains_strings =
                 count(line.begin(), line.end(), C_STRING_DELIMITER)) > 0) {
...
void CSVData::write_data(const string &filename) {
    ...
    if (!output_file.is_open()) {
        cerr << "Unable to open file '" << filename
             << "'. Check if you have the right permissions." << endl;
        return;
    }
...
void CSVData::convert_date_format(const string &old_format,
                                  const string &new_format, int column) {
    for (int row = 0; row < (int)m_data.size(); ++row)
...
void CSVData::convert_date_format(const string &old_format,
                                  const string &new_format, int row,
                                  int column) {
...
void CSVData::sort_by_col(int col, int order) {
    auto acs_compare_fun = [&](const vector<string> &row1,
                               const vector<string> &row2) {
        bool ret_val;

        if (is_digits(row1[col]) && is_digits(row2[col])) {
            auto a = (row1[col].find(DECIMAL_DELIMITER) != string::npos)
                         ? stod(row1[col])
                         : stol(row1[col]);
            auto b = (row2[col].find(DECIMAL_DELIMITER) != string::npos)
                         ? stod(row2[col])
                         : stol(row2[col]);
            ret_val = a < b;
        } else {
    ...
    auto desc_compare_fun = [&](const vector<string> &row1,
                                const vector<string> &row2) {
        bool ret_val;
